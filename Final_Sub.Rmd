...
---
###This code has been written to perform the given task on the data provided to me in a report using a single R markdown document that can be processed by knitr and be transformed into an HTML file.Since the data set has such a great size and we need make a class prediction, random forests model is implemented. The test set error is estimated internally, during the execution. So, we proced with the training the model (Random Forest) with the training data set. Further details are mentioned in the code. The parallel package provides functions for parallel execution of R code on machines with multiple cores or processors or multiple computers. It is essentially a blend of the snow and multicore packages. By default, the doParallel package uses snow-like functionality. The snow-like functionality should work fine on Unix-like systems.
